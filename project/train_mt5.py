...
def train_iteration(model, optimizer, dataset, train_pairs, qrels):
    ...
    with tqdm('training', total=BATCH_SIZE * BATCHES_PER_EPOCH, ncols=80, desc='train') as pbar:
        for record in data.iter_train_pairs_with_labels(model, dataset, train_pairs, qrels, GRAD_ACC_SIZE):
            loss, logits = model(record['query_tok'],
                                 record['query_mask'],
                                 record['doc_tok'],
                                 record['doc_mask'],
                                 record['labels'])
            # extra_labels = record['extra_labels']
            ...
            # tf_scores = tf_logits.softmax(dim=-1)
            # # loss_fn = torch.nn.CrossEntropyLoss()
            # # loss_cross = loss_fn(tf_scores, extra_labels.view(-1))
            # true_scores = tf_scores[:, 0]
            # false_scores = tf_scores[:, 1]
            # true_scores = true_scores.reshape(count, 2)
            # true_loss = torch.mean(1. - true_scores.softmax(dim=1)[:, 0])
            # false_scores = false_scores.reshape(count, 2)
            # false_loss = torch.mean(1. - false_scores.softmax(dim=1)[:, 1])
...
def main_cli():
    parser = argparse.ArgumentParser('MT5 model training and validation')
