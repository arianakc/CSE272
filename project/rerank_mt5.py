...
def main_cli():
    ...
    parser.add_argument('--model', choices=train_mt5.MODEL_MAP.keys(), default='vanilla_mt5')
    ...
    model = train_mt5.MODEL_MAP[args.model]().cuda()
    dataset = data.read_datafiles(args.datafiles)
    run = data.read_run_dict(args.run)
    if args.model_weights is not None:
        model.load(args.model_weights.name)
    rerank_run = train_mt5.run_model(model, dataset, run, desc='rerank')
    train_mt5.write_run(rerank_run, args.out_path.name)
